export default defineUnlistedScript(() => {
  ...
  window.addEventListener('message', async (event) => {
    ...
    if (event.data.type === 'bloom:getPublicKey') {
      if (!nostr) {
        window.postMessage({ channel: CHANNEL, type: 'bloom:error', id: event.data.id, error: 'NIP-07 signer not found' }, '*');
        return;
      }
      try {
    ...
    if (event.data.type === 'bloom:signEvent') {
      ...
      try {
        ...
        window.postMessage({ channel: CHANNEL, type: 'bloom:signedEvent', id: event.data.id, data: signed }, '*');
      } catch (err) {
        window.postMessage({ channel: CHANNEL, type: 'bloom:error', id: event.data.id, error: String(err) }, '*');
      }
    }
